--[[
TEN WEAK // ECLIPSE PROTOCOL HUB v4.0 (DELTA OPTIMIZED)
...
local State = {
    ...
    JumpPower = 50,
    Spectating = false,
    GameName = string.lower(game.Name),
    OriginalLighting = {}
}
...
local function SafeBind(name, priority, func)
    pcall(function() RunService:UnbindFromRenderStep("TW_" .. name) end)
    RenderBindings[name] = RunService:BindToRenderStep("TW_" .. name, priority, func)
end

local function SafeUnbind(name)
    if RenderBindings[name] then
        pcall(function() RunService:UnbindFromRenderStep("TW_" .. name) end)
...
local function EnableGodMode()
    ...
    SafeBind("GodMode", Enum.RenderPriority.Last.Value, function()
        local h = GetHumanoid()
...
local function ToggleFly(Active)
    if Active then
        SafeBind("Fly", Enum.RenderPriority.Input.Value, function()
            local root = GetRoot()
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if State.ActiveModes["NoClip"] then
        SafeBind("NoClip", Enum.RenderPriority.First.Value, function()
            local c = SafeGetChar()
...
StatusText.Text = "Build: v4.0.0 | Connected | Servers Online: 4,782 | Users Online: 23,456"
...
local function CreateButton(Parent, Name, Callback)
    ...
    Btn.MouseEnter:Connect(function() TweenService:Create(Btn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(40, 40, 55)}):Play() end)
    Btn.MouseLeave:Connect(function() TweenService:Create(Btn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(30, 30, 42)}):Play() end)
end
...
RunService.RenderStepped:Connect(function()
    ...
    if hum and root then
        local hp = hum.Health == math.huge and "INF" or math.floor(hum.Health)
        local maxhp = hum.MaxHealth == math.huge and "INF" or math.floor(hum.MaxHealth)
        local ws = hum.WalkSpeed == math.huge and "INF" or math.floor(hum.WalkSpeed)
        local jp = hum.JumpPower == math.huge and "INF" or math.floor(hum.JumpPower)
        StatsLabel.Text = string.format("HP: %s/%s\nWS: %s | JP: %s\nPOS: %.1f, %.1f, %.1f\nGAME: %s",
            hp, maxhp, ws, jp,
...
CreateButton(Combat, "Fake Chat Spam (UTF-8)", function()
    local chars = "█░▒▓"
    for i = 1, 50 do game:GetService("Chat"):Chat(LocalPlayer, string.rep(chars, 20), Enum.ChatColor.White) end
end)
...
CreateToggle(Visuals, "Chams ESP (Highlight)", false, function(Active)
    if Active then
        SafeBind("ESP", Enum.RenderPriority.Camera.Value, function()
            for _, player in ipairs(Players:GetPlayers()) do
...
CreateToggle(Modes, "Statue (Anti-Kill/Freeze)", false, function(Active)
    if Active then
        SafeBind("Statue", Enum.RenderPriority.Last.Value, function()
            local hum = GetHumanoid()
            if hum then 
...
CreateButton(Settings, "Destroy GUI", function()
    for name, _ in pairs(RenderBindings) do SafeUnbind(name) end
    Gui:Destroy()
end)

--// SYSTEM INITIALIZATION //--
InitACBypass()
Notify("TEN WEAK // ECLIPSE PROTOCOL HUB v4.0 FULLY COMPILED.")
